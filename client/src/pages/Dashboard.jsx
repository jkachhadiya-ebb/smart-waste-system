...
const ATAKUM_MAP_CENTER = { lat: 41.338039, lng: 36.272007 };
const ATAKUM_RECYCLING_CENTER = {
  lat: 41.3136983,
  lng: 36.2671813,
  label: 'Hobi Bahçeleri Karşısı, Çobanlı, Aydınlık Cd., 55200 Atakum/Samsun, Türkiye',
};
const ATAKUM_MAP_BOUNDS = [[41.31, 36.215], [41.375, 36.315]];
const EMPTY_ARRAY = [];
...
export default function Dashboard() {
  ...
  const [_summary, setSummary] = useState({ wasteKg: 0, co2Kg: 0, coKg: 0 });
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
        ...
        if (Array.isArray(liveTrucks) && liveTrucks.length > 0) {
          ...
          liveTrucks.forEach((tr) => {
            ...
            initialTrucks[tr.truckId] = {
              ...
              municipalityId: tr.municipalityId || null,
              municipalityCode: tr.municipalityCode || null,
              municipalityName: tr.municipalityName || null,
              city: tr.city || null,
              country: tr.country || null,
              streetName: tr.streetName || null,
              lat: tr.lat,
              lng: tr.lng,
      ...
      const handleTelemetry = (data) => {
        ...
        const municipalityCode = data.municipalityCode || data.municipality_code || null;
        const isAtakum = municipalityCode === 'TR-SAM-ATAKUM';
        if (
          isAtakum &&
          (lat < ATAKUM_MAP_BOUNDS[0][0] || lat > ATAKUM_MAP_BOUNDS[1][0] ||
            lng < ATAKUM_MAP_BOUNDS[0][1] || lng > ATAKUM_MAP_BOUNDS[1][1])
        ) return;
        ...
        if (!isAtakum && allowOptimized && Array.isArray(optimizedRoute) && optimizedRoute.length > 1) {
        ...
        const telemetry = {
          ...
          municipalityId: data.municipalityId || data.municipality_id || null,
          municipalityCode,
          municipalityName: data.municipalityName || data.municipality_name || null,
          city: data.city || null,
          country: data.country || null,
          streetName: data.streetName || data.street_name || null,
          lat: displayLat,
          lng: displayLng,
          ...
          sourceTimestamp: data.sourceTimestamp || data.source_timestamp || null,
        };
        ...
        setPoints((prev) =>
          [
            ...prev,
            {
              ...
              municipalityCode: telemetry.municipalityCode,
              city: telemetry.city,
  ...
  const cityChartPoints = useMemo(() => {
    const aggregate = (rows) => {
      const grouped = new Map();
      rows.forEach((point) => {
        if (hiddenTruckIds.has(String(point.truckId))) return;
        const key = point.time || point.timestamp;
        ...
        current.wasteKg += Number(point.wasteKg) || 0;
        current.co2Kg += Number(point.co2Kg) || 0;
        current.coKg += Number(point.coKg) || 0;
        grouped.set(key, current);
      });
      return Array.from(grouped.values()).slice(-50);
    };

    return {
      koblenz: aggregate(points.filter((point) => point.municipalityCode !== 'TR-SAM-ATAKUM')),
      atakum: aggregate(points.filter((point) => point.municipalityCode === 'TR-SAM-ATAKUM')),
    };
  }, [points, hiddenTruckIds]);
  ...
  const atakumLiveTrucks = useMemo(
    () => normalizedTrucks.filter((truck) => truck.municipalityCode === 'TR-SAM-ATAKUM'),
    [normalizedTrucks]
  );

  const koblenzTrucks = useMemo(
    () => normalizedTrucks.filter((truck) => truck.municipalityCode !== 'TR-SAM-ATAKUM'),
    [normalizedTrucks]
  );
  ...
  const mapTrucks = useMemo(
    () => (showLiveTrucks ? koblenzTrucks : EMPTY_ARRAY),
    [showLiveTrucks, koblenzTrucks]
  );
  ...
  const summarizeCityKpis = (cityTrucks) => ({
    wasteKg: Number(cityTrucks.reduce((sum, tr) => sum + (Number(tr?.wasteKg) || 0), 0).toFixed(2)),
    co2Kg: Number(cityTrucks.reduce((sum, tr) => sum + (Number(tr?.co2Kg) || 0), 0).toFixed(2)),
    coKg: Number(cityTrucks.reduce((sum, tr) => sum + (Number(tr?.coKg) || 0), 0).toFixed(2)),
  });
  const koblenzKpis = summarizeCityKpis(koblenzTrucks);
  const atakumKpis = summarizeCityKpis(atakumLiveTrucks);
  ...
  function renderChart(chartData) {
    const renderAxes = () => [
      ...
      <YAxis
        ...
        label={{ value: t('Emissions (kg)'), angle: -90, position: 'insideLeft', style: { fill: 'var(--chart-axis)', fontSize: 10 } }}
      />,
      <YAxis
        key="yr"
        yAxisId="right"
        ...
        label={{ value: t('Waste (kg)'), angle: 90, position: 'insideRight', style: { fill: 'var(--chart-axis)', fontSize: 10 } }}
      />,
      <Tooltip
        key="tooltip"
        cursor={{ stroke: '#0ea5e9', strokeWidth: 1 }}
        contentStyle={{ color: 'var(--text-primary)', background: 'var(--chart-tooltip-background)', borderRadius: 10, border: '1px solid var(--chart-axis-line)', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.2)' }}
      />,
    ...
    <div className="dashboard-container">
      ...
      <div className="top-section">
        <div className="panel map-panel">
          ...
          <div className="dual-map-grid">
            <section className="city-map">
              <div className="city-map__title">{t('Koblenz, Germany')}</div>
            ...
            <section className="city-map">
              <div className="city-map__title">{t('Atakum, Turkey')}</div>
              <MapContainer
                center={[ATAKUM_MAP_CENTER.lat, ATAKUM_MAP_CENTER.lng]}
                zoom={13}
                minZoom={12}
                maxBounds={ATAKUM_MAP_BOUNDS}
              ...
              >
                ...
                <MapAutoCenter
                  selectedTruckId={selectedTruck}
                  trucksList={atakumLiveTrucks}
                  optimizedRoutes={EMPTY_ARRAY}
                  mapView="municipality"
                  mapZoom={Math.max(13, mapZoom)}
                  baseCenter={ATAKUM_MAP_CENTER}
                  showLiveTrucks={showLiveTrucks}
                  registeredCities={EMPTY_ARRAY}
                  showRegisteredCities={false}
                />
                {showRecyclingCenters && (
                  <Marker
                    position={[ATAKUM_RECYCLING_CENTER.lat, ATAKUM_RECYCLING_CENTER.lng]}
                    icon={markerStyle === 'labels' ? recyclingLabelIcon : recyclingIcon}
                  >
                    <Popup>
                      <strong>{t('Recycling centre')}</strong>
                      <br />
                      {ATAKUM_RECYCLING_CENTER.label}
                    </Popup>
                  </Marker>
                )}
                {showLiveTrucks &&
                  atakumLiveTrucks.map((tr) => {
                    const path = truckPaths[String(tr.truckId)] || [];
                    const labelIcon = truckLabelIcons.get(String(tr.truckId));
                    const icon = markerStyle === 'labels' ? labelIcon || truckIcon : truckIcon;
                    return (
                      <Fragment key={`atakum-truck-${tr.truckId}`}>
                        {path.length > 1 && (
                          <Polyline
                            positions={path}
                            pathOptions={{
                              color: trailColorFor(tr.truckId),
                              weight: 4,
                              opacity: 0.82,
                            }}
                          />
                        )}
                        <Marker position={[tr.lat, tr.lng]} icon={icon}>
                          <Popup>
                            <strong>{tr.type || t('Truck {{id}}', { id: tr.truckId })}</strong>
                            <br />
                            {tr.streetName || t('Atakum')}
                            <br />
                            {t('Speed: {{value}} km/h', { value: tr.speedKmh ?? '-' })}
                            <br />
                            {t('Waste: {{value}} kg', { value: tr.wasteKg ?? '-' })}
        ...
        <div className="right-column">
          <div className="municipality-kpi-grid">
            {[
              { name: t('Koblenz, Germany'), values: koblenzKpis },
              { name: t('Atakum, Turkey'), values: atakumKpis },
            ].map((city) => (
              <section className="municipality-kpis" key={city.name}>
                <div className="municipality-kpis__title">{city.name}</div>
                <div className="kpi-row">
                  <div className="kpi-card">
                    <div className="kpi-label">{t('Waste load')}</div>
                    <div className="kpi-value">{city.values.wasteKg} <span>{t('kg')}</span></div>
                  </div>
                  <div className="kpi-card">
                    <div className="kpi-label">{t('CO₂ Emission')}</div>
                    <div className="kpi-value">{city.values.co2Kg} <span>{t('kg')}</span></div>
                  </div>
                  <div className="kpi-card">
                    <div className="kpi-label">{t('CO Emission')}</div>
                    <div className="kpi-value">{city.values.coKg} <span>{t('kg')}</span></div>
                  </div>
                </div>
              </section>
            ))}
          ...
          <div className="chart-header municipality-chart-header">
            <div className="chart-title">{t('Live emissions & waste charts')}</div>
          ...
          <div className="municipality-chart-grid">
            {[
              { name: t('Koblenz, Germany'), data: cityChartPoints.koblenz },
              { name: t('Atakum, Turkey'), data: cityChartPoints.atakum },
            ].map((city) => (
              <section className="panel chart-panel municipality-chart-panel" key={city.name}>
                <div className="municipality-kpis__title">{city.name}</div>
                <div className="chart-wrapper">
                  <ResponsiveContainer width="100%" height={300} minWidth={200}>
                    {renderChart(city.data)}
                  </ResponsiveContainer>
                </div>
              </section>
            ))}
